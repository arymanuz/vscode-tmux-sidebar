...
function scopedValue(key, scope) {
    const inspected = vscode.workspace.getConfiguration('tmuxSidebar').inspect(key);
    if (!inspected) {
        return undefined;
    }
...
function readSettings(scope) {
    let programs = (0, programs_1.sanitizeSpecs)(scopedValue('programs', scope));
    if (programs.length === 0) {
        // A value so broken nothing survives is shown as the default list.
        programs = (0, programs_1.sanitizeSpecs)(vscode.workspace.getConfiguration('tmuxSidebar').inspect('programs')?.defaultValue);
    }
    return {
        autoRefreshEnabled: scopedValue('autoRefresh.enabled', scope) ?? true,
        autoRefreshInterval: scopedValue('autoRefresh.intervalSeconds', scope) ?? 3,
        terminalLocation: scopedValue('newTerminalLocation', scope) ?? 'vscode',
        programs
    };
}
let current;
